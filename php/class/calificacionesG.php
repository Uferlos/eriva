...
class Calificaciones implements Interfaces{
  ...
  public function __construct($ci_a, $id_a, $notan, $notal, $te, $mes, $anio, $id_p, $db){
    $this->ci_a = $ci_a;
    $this->id_a = $id_a;
    $this->notan = $notan;
    $this->notal = $notal;
  ...
  public function Add(){
    ...
    for($i = 0; $i < count($this->id_a); $i++):
      $select = "SELECT * FROM calificacionesG WHERE ci_alumno = '$this->ci_a' AND id_asignatura = '{$this->id_a[$i]}'";
      if(!$res_sel = $this->db->query($select)){
        echo $this->db->error;
        exit();
      }
      if(!$res_sel->num_rows):
        $query = "INSERT INTO calificacionesG VALUES(null,
        '$this->ci_a', '{$this->id_a[$i]}' , '{$this->notan[$i]}',
        '{$this->notal[$i]}', '{$this->te[$i]}', '{$this->mes[$i]}',
        '{$this->anio[$i]}', '{$this->id_p[$i]}')";
        $res = $this->db->query($query);
        if(!$res):
          $aux++;
        endif;
      else:
        $query = "UPDATE calificacionesG SET
        notan = '{$this->notan[$i]}', 
        notal = '{$this->notal[$i]}', 
        te = '{$this->te[$i]}', 
        mes = '{$this->mes[$i]}',
...
$obj = new Calificaciones($ci_a, $id_a, $notan, $notal, $te, $mes, $anio, $id_p, $db);
